...
export default function Checkout({ productList }) {
  ...
  const [paymentMode, setPaymentMode] = useState("Trả trước");
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (!productList || productList?.length === 0) {
        throw new Error("Danh sách sản phẩm đang trống");
      }

      if (paymentMode === "Trả trước") {
        ...
        <section className="flex flex-col gap-3 border rounded-xl p-4">
          ...
          <div className="flex flex-col gap-2">
            {productList?.map((item, index) => {
              return (
                <div className="flex gap-3 items-center" key={item?.product?.id || index}>
                  <img
        ...
        <section className="flex flex-col gap-3 border rounded-xl p-4">
          <div className="flex flex-col md:flex-row items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={() => {
                  setPaymentMode("Trả trước");
                }}
                className="flex items-center gap-1 text-xs"
              >
                {paymentMode === "Trả trước" && (
                  <CheckSquare2Icon className="text-blue-500" size={13} />
                )}
                {paymentMode === "Trả Sau" && <Square size={13} />}
                Trả trước
              </button>
              <button
                onClick={() => {
                  setPaymentMode("Trả Sau");
                }}
                className="flex items-center gap-1 text-xs"
              >
                {paymentMode === "Trả trước" && <Square size={13} />}
                {paymentMode === "Trả Sau" && (
                  <CheckSquare2Icon className="text-blue-500" size={13} />
                )}
                Thanh toán khi nhận
          ...
          <div className="flex gap-1 items-center">
            ...
            Tôi đồng ý với{" "}
              <span className="text-blue-700">điều khoản và điều kiện</span>
            </h4>
          </div>
          <Button
            isLoading={isLoading}
